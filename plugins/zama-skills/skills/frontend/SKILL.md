...
- **Use the `/web` subpath, NOT `/bundle`.** `@zama-fhe/relayer-sdk/web` is the self-contained ESM build that bundlers (Vite, webpack) can tree-shake correctly — `initSDK`, `createInstance`, `SepoliaConfig` are all real named exports. `/bundle` is for CDN-style loading via a separate `<script>` tag that pre-sets `window.relayerSDK`; under a bundler its named exports resolve to `window.relayerSDK.initSDK` which is `undefined` at module load → `TypeError: Cannot read properties of undefined (reading 'initSDK')`.
- **The host page MUST serve `Cross-Origin-Opener-Policy: same-origin` + `Cross-Origin-Embedder-Policy: credentialless` headers.** `@zama-fhe/relayer-sdk/web` uses WebAssembly threads → SharedArrayBuffer → Cross-Origin Isolation. Without these headers `initSDK()` silently fails and every downstream call throws "no cached instance". Vite dev: configure `server.headers` + `preview.headers` in `vite.config.ts`. Vercel/static prod: ship a `vercel.json` (or `_headers` for Netlify/Cloudflare). Next.js: add a `headers()` entry in `next.config.mjs`. The `/zama-init` scaffold ships these by default — do not strip them.
- **`userDecrypt` signature is positional:** `userDecrypt(handleContractPairs, privateKey, publicKey, signature, contractAddresses, userAddress, startTimestamp, durationDays)`. The `signature` MUST be `0x`-stripped (`.replace(/^0x/, "")`). The Zama webapp guide shows `startTimestamp`/`durationDays` as **strings** (`Math.floor(Date.now()/1000).toString()`, `"10"`); a few earlier hardhat-template snippets used numbers. Passing the *opposite type* than what the installed SDK build expects throws `InvalidTypeError: ... UintNumber`. Match the type the docs example uses for your SDK version, or coerce defensively (`String(startTimestamp)` / `Number(durationDays)`) — see the `/zama-debug` pattern `relayer-sdk-eip712-type-mismatch`.
